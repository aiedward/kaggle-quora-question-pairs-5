...
STAMP = 'lstm_%d_%d_%.2f_%.2f_%s'%(num_lstm, num_dense, rate_drop_lstm, \
        rate_drop_dense, dtime)

# Load data
(data_1_full, data_2_full, labels_full), (test_data_1, test_data_2, test_ids) = cPickle.load(open("data.pkl", "rb"))
...
# Use only train samples from xgb
train_idx = np.load('train_idx.npy')
data_1 = data_1_full[train_idx]
data_2 = data_2_full[train_idx]
labels = labels_full[train_idx]
...
hist = model.fit([data_1_train, data_2_train], labels_train, \
        ...
        epochs=7, batch_size=2086, shuffle=True, \
...
print('Save train predictions')
preds = model.predict([data_1_full, data_2_full], batch_size=1024, verbose=1)

submission = pd.DataFrame({'lstm_oof':preds.ravel()})
...
submission.to_csv('submits_lstm/%.4f_'%(bst_val_score)+STAMP+'.csv', index=False)